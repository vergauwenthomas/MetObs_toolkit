...
Next, quality control is performed in the form of a series of checks [@metobs_qc]. These checks are designed to examine data types, irregular timestamps, max-min thresholds, repetitions criteria, spike tests, allowed variation in time windows and spatial tests [@metobs_buddy]. Advanced quality control methods are available through the implementation of TITAN into the toolkit. The user can choose to keep the outliers or convert them to missing records (which can then be filled).

Finally, gap filling is applied by using interpolation methods [@metobs_interpolation] and/or importing ERA5 reanalysis [@era5] time series to replace missing records [@metobs_gapfill]. The latter is stored as a toolkit *Modeldata class* instance, which has a set of methods to directly import the required time series through the use of the Google Earth Engine API. The user then obtains a cleaned-up dataset ready for analysis. A set of typical analysis techniques such as filters, aggregation schemes, and landcover correlation estimates are implemented in the toolkit *Analysis class*.
