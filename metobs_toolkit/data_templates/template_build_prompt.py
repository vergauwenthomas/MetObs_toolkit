...
def build_template_prompt():
    ...
    datetime_option = int(
        input(
            " How are the timestamps represented in your dataset: \n \
    ...
    print("\n \n *******      Meta Data File   ***********")
    metatemplate_dict = {}
    meta_avail = yes_no_ques(
        "The metadata contains metadata for each station, where each column represent a metadata type. \n Do you have a metadatafile?"
    )
    if meta_avail:
    ...
    if apply_tips is True:
        ...
        if format_option == 2:
            ...
            print(
                f'    obstype_description="{template_dict[wide_obstype]["description"]}",'
            )
        ...
        print(
            '    freq_estimation_method=None, #None for default(="highest"), "highest" or "median"'
        )
        print(
            "    freq_estimation_simplify=None, #None for default(=True), True or False"
        )
        print(
            '    freq_estimation_simplify_error=None, #None for default(="2T"), or timedelta string.'
        )
