...
def _make_pie_from_freqs(freq_dict, colormapper, ax, plot_settings, title=None):
    # To dataframe
...
def qc_stats_pie(
    ...
    _make_pie_from_freqs(
        final_stats, final_col_mapper, ax_thl, plot_settings, "Final label frequencies"
    )
    ...
    _make_pie_from_freqs(
        outlier_stats, outl_col_mapper, ax_thr, plot_settings, "Outlier performance"
    )
    ...
    for checkname, stats in specific_stats.items():
        ...
        _make_pie_from_freqs(stats, spec_col_mapper, ax, plot_settings, checkname)
        i += 1
