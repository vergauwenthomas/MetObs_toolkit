...
def folium_plot(mapinfo, band, vis_params, labelnames, layername,
    ...
    if legendname:
        MAP.add_legend(title=legendname,
                       labels=labelnames,
...
def timeseries_plot(mergedf, title, ylabel, colorby, show_legend,
                    show_outliers, show_filled, settings,
...
>>>>>>> master
    ...
    if colorby == "label":
        ...
        for sta in mergedf.index.get_level_values('name').unique():
            ...
            sta_line_lc = _create_linecollection(linedf=stadf,
...
def model_timeseries_plot(df, obstype, title, ylabel, settings,
                          show_primary_legend, add_second_legend=True,
                          _ax=None,  # needed for GUI, not recommended use
...
def cycle_plot(cycledf, errorbandsdf, title, plot_settings,
                 aggregation, data_template, obstype, y_label,
    ...
    if legend:
        ...
        ax.set_position([box.x0, box.y0 + box.height * 0.2,
                         box.width, box.height * 0.88])
        ax.legend(cycledf.columns.values.tolist(), loc='upper center',
                  bbox_to_anchor=(0.5, -0.2),
                  fancybox=True, shadow=True,
...
def _make_pie_from_freqs(freq_dict, colormapper, ax, plot_settings,
                         radius, labelsize=10):
