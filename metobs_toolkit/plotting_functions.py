...
def timeseries_plot(
    ...
    if colorby == "label":
        ...
        missing_fill_labels = [
            val
            for val in settings.missing_obs["missing_obs_fill_info"]["label"].values()
        ]
...
def diurnal_plot(
    ...
    if colorby == "lcz":
        ...
        if legend:
            box = ax.get_position()
            ax.set_position(
                [box.x0, box.y0 + box.height * 0.2, box.width, box.height * 0.95]
            )
            ax.legend(
                handles=custom_handles,
                loc="upper center",
    ...
    if colorby == "name":
        ...
        if legend:
            ...
            ax.legend(
                diurnaldf.columns.values.tolist(),
                loc="upper center",
                bbox_to_anchor=(0.5, -0.2),
                fancybox=True,
                shadow=True,
