...
# def window_variation_check(records:pd.Series,
#                             timewindow:pd.Timedelta,
#                             min_records_per_window:int,
#                            max_increase_per_second:int|float,
#                            max_decrease_per_second:int|float) -> pd.DatetimeIndex:
#     """Test if the variation exceeds the threshold in moving time windows.
...
#     ismet = _test_moving_window_condition(records=records,
#                                           windowsize=timewindow,
#                                           min_records_per_window=min_records_per_window)
#     if not ismet:
#         logger.warning('The minimum number of window members for the window variation check is not met!')
...
#     # Calculate window thresholds (by linear extarpolation)
#     max_window_increase = float(abs(max_increase_per_second)) * timewindow.total_seconds()
#     max_window_decrease = float(abs(max_decrease_per_second)) * timewindow.total_seconds()
