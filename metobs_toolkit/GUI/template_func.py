...
Obs_map_values = {
    ...
    "radiation_temp": {
        "units": ["K", "Celcius"],
        "description": "Blackglobe temperature",
        "dtype": "float64",
    },
    "humidity": {
        "units": ["%"],
    ...
    "precip_sum": {
        "units": ["l/m²", "Celcius"],
        "description": "Precipitation cumulated",
        "dtype": "float64",
    },
    "wind_speed": {
        "units": ["m/s"],
    ...
    "wind_direction": {
        "units": ["°"],
        "description": "Wind direction (from north CW)",
        "dtype": "float64",
    },
    "pressure": {"units": ["Pa"], "description": "Air pressure", "dtype": "float64"},
    "pressure_at_sea_level": {
        "units": ["Pa"],
        "description": "Pressure at sealevel",
        "dtype": "float64",
    },
}
...
def make_template_build(main):
    ...
    def get_obs_map(map_CB, unit_CB, desc_T):
        ...
        if mapcolname != not_mapable:
            ...
            returndict = {"map_column": np.nan, "map_unit": np.nan, "map_desc": np.nan}

        return returndict

    # add obsmapper to obsmapperdict
    obsmapper["temp"] = get_obs_map(
        main.temp_col_CB, main.temp_units_CB, main.temp_desc_T
    )
    obsmapper["radiation_temp"] = get_obs_map(
        main.radtemp_col_CB, main.radtemp_units_CB, main.radtemp_desc_T
    )

    obsmapper["humidity"] = get_obs_map(
        main.hum_col_CB, main.hum_units_CB, main.hum_desc_T
    )
    obsmapper["precip"] = get_obs_map(
        main.pre_col_CB, main.pre_units_CB, main.pre_desc_T
    )
    obsmapper["precip_sum"] = get_obs_map(
        main.pre_s_col_CB, main.pre_s_units_CB, main.pre_s_desc_T
    )
    obsmapper["wind_speed"] = get_obs_map(
        main.wind_col_CB, main.wind_units_CB, main.wind_desc_T
    )
    obsmapper["wind_gust"] = get_obs_map(
        main.gust_col_CB, main.gust_units_CB, main.gust_desc_T
    )
    obsmapper["wind_direction"] = get_obs_map(
        main.dir_col_CB, main.dir_units_CB, main.dir_desc_T
    )
    obsmapper["pressure"] = get_obs_map(main.p_col_CB, main.p_units_CB, main.p_desc_T)
    obsmapper["pressure_at_sea_level"] = get_obs_map(
        main.psea_col_CB, main.psea_units_CB, main.psea_desc_T
    )
    ...
    def get_dt_map(map_CB, fmt_T):
        mapcolname = str(map_CB.currentText())
        if mapcolname != not_mapable:
            returndict = {"map_column": mapcolname, "map_fmt": str(fmt_T.text())}
        else:
            returndict = {"map_column": np.nan, "map_fmt": np.nan}

        return returndict

    dtmapper["datetime"] = get_dt_map(main.datetime_col_CB, main.datetime_fmt_T)
    dtmapper["_date"] = get_dt_map(main.date_col_CB, main.date_fmt_T)
    dtmapper["_time"] = get_dt_map(main.time_col_CB, main.time_fmt_T)
    ...
    def to_dataframe(mapper, dtype):
        ...
        df = df.rename(
            columns={
                "map_column": "template column name",
                "map_unit": "units",
