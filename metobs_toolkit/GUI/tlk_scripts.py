    ...
    for checkname in qc_set:
        ...
        for set_name, set_val in check_set.items():
            ...
            if widgetname in tlk_to_gui:
                ...
                if apl == "multiply":
                    set_val = _multiply(set_val, val)
                elif apl == "devide":
                    set_val = _div(set_val, val)
                elif apl == "append":
                    set_val = _append(set_val, val)
                elif apl == "remove":
                    set_val = _remove(set_val, val)
    ...
    for checkname in qc_set:
        ...
        for set_name, _ in check_set.items():
            ...
            if widgetname in gui_to_tlk:
                ...
                if apl == "multiply":
                    set_val = _multiply(widget_val, val)
                elif apl == "devide":
                    set_val = _div(widget_val, val)
                elif apl == "append":
                    set_val = _append(widget_val, val)
                elif apl == "remove":
                    set_val = _remove(widget_val, val)
            else:
                set_val = widget_val
            # set value
            dataset.settings.qc["qc_check_settings"][checkname][obstype][
                set_name
            ] = set_val
