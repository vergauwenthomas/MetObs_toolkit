...
sys.path.insert(
    0, "/home/thoverga/Documents/VLINDER_github/MetObs_toolkit/metobs_toolkit"
)
...
class MainWindow(QDialog):
    def __init__(self):
        ...
        # Load gui widgets
        loadUi(
            os.path.join(path_handler.GUI_dir, "templ_build.ui"), self
        )  # open the ui file
        ...
        # ------- Callbacks (Getters) ---------
        self.Browse_data_B.clicked.connect(
            lambda: self.browsefiles_data()
        )  # browse datafile
        self.Browse_data_B_2.clicked.connect(
            lambda: self.browsefiles_data_p2()
        )  # browse datafile
        self.Browse_metadata_B.clicked.connect(
            lambda: self.browsefiles_metadata()
        )  # browse metadatafile
        self.Browse_metadata_B_2.clicked.connect(
            lambda: self.browsefiles_metadata_p2()
        )  # browse metadatafile

        # ------- Callbacks (triggers) ------------
        # P1 ------
        # save paths when selected
        self.save_data_path.clicked.connect(
        ...
        self.save_metadata_path.clicked.connect(
            lambda: self.save_path(
    ...
    def browsefiles_metadata(self):
        fname = QFileDialog.getOpenFileName(
            self, "Select metadata file", str(Path.home())
        )
...
if __name__ == "__main__":
    matplotlib.use(
        "Qt5Agg"
    )  # in protector because server runners do not support this, when this module is imported from the __init__
    app = QApplication(sys.argv)
