...
def add_final_label_to_outliersdf(
    outliersdf, data_res_series, observation_types, checks_info
):
    """
    V3
    ...
    # order columns
    labels_columns = [
        column for column in outliersdf.columns if not column in observation_types
    ]
    # drop final columns if they are in the outliersdf
    labels_columns = [
        column for column in labels_columns if not column.endswith("_final_label")
    ]

    checked_obstypes = [
        obstype
        for obstype in observation_types
        if any([qc_column.startswith(obstype + "_") for qc_column in labels_columns])
    ]
    columns_on_record_lvl = [
        info["label_columnname"]
        for checkname, info in checks_info.items()
        if info["apply_on"] == "record"
    ]

    # Construct numeric mapper
    labels_to_numeric_mapper = {
        info["outlier_flag"]: info["numeric_flag"] for info in checks_info.values()
    }
    ...
    for obstype in checked_obstypes:
        ...
        specific_columns = [
            col for col in labels_columns if col.startswith(obstype + "_")
        ]
        ...
        specific_columns = [
            colmn for colmn in specific_columns if colmn in outliersdf.columns
        ]
        ...
        outliersdf[obstype + "_final_label"] = num_qc_df.sum(axis=1, skipna=True).map(
            inv_label_to_num
        )
