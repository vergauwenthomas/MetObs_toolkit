    ...
    *{self.missing_obs.series.shape[0]} missing observations \n"
    ...
    def make_plot(
        ...
        if not show_outliers:
            outlier_labels = [
                var["outlier_flag"]
                for var in self.settings.qc["qc_checks_info"].values()
            ]
        ...
        if title is None:
            if stationnames is None:
                if self._istype == "Dataset":
                    title = (
                        self.data_template[obstype]["orig_name"] + " for all stations. "
                    )
                elif self._istype == "Station":
                    title = (
                        self.data_template[obstype]["orig_name"] + " of " + self.name
                    )
    ...
    def apply_quality_control(
        ...
        if repetitions:
            ...
            if apliable:
                ...
                print(
                    f"ERROR: The repetitions check can NOT be applied on {obstype} because it was already applied on this observation type!"
                )
        ...
        if gross_value:
            ...
            if apliable:
                ...
                print(
                    f"ERROR: The gross_value check can NOT be applied on {obstype} because it was already applied on this observation type!"
                )
        ...
        if persistance:
            ...
            if apliable:
                ...
                print(
                    f"ERROR: The persistance check can NOT be applied on {obstype} because it was already applied on this observation type!"
                )
        ...
        if step:
            ...
            if apliable:
                ...
                print(
                    f"ERROR: The step check can NOT be applied on {obstype} because it was already applied on this observation type!"
                )
        ...
        if window_variation:
            ...
            if apliable:
                ...
                print(
                    f"ERROR: The window_variation check can NOT be applied on {obstype} because it was already applied on this observation type!"
                )
    ...
    def apply_titan_buddy_check(self, obstype="temp", use_constant_altitude=False):
        ...
        if self.metadf["lat"].isnull().any():
            print(
                f"ERROR: Not all coordinates are available, the {checkname} cannot be executed!"
            )
            return
        if self.metadf["lon"].isnull().any():
            print(
        ...
        if (not use_constant_altitude) & ("altitude" not in self.metadf.columns):
            print(
                f"ERROR: The altitude is not known for all stations. The {checkname} cannot be executed!"
            )
            print(
        ...
        if apliable:
            ...
            print(
                f"ERROR: The {checkname} can NOT be applied on {obstype} because it was already applied on this observation type!"
            )
    ...
    def coarsen_time_resolution(
        ...
        if (
            ...
            print(
                "WARNING: Coarsening time resolution is not possible because quality control checks that are resolution depening are already performed on the Dataset."
            )
    ...
    def make_gee_plot(self, gee_map, show_stations=True, save=False):
        ...
        if save:
            if self.settings.IO["output_folder"] is None:
                print(
                    "WARNING: The outputfolder is not set up, use the update_settings to specify the output_folder."
                )
