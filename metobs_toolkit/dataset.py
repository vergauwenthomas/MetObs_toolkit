...
class Dataset:
    ...
    *time zone of the records: {self.settings.time_settings['timezone']} \n "
    ...
    def make_interactive_plot(
        ...
        for val in self.settings.gap["gaps_fill_info"]["methods"]:
            label_col_map[val] = fill_col
    ...
    def combine_all_to_obsspace(
        ...
        # gapsdf contains unfilled gaps + filled gaps
        # TODO: Some gaps can be partially filled, so how to intpret the records
        # of a gap that could not be filled, but gapfill is applied?

        # 1. gap records that are not filled (or tried to be filled)
        pure_gaps = gapsdf.loc[gapsdf["fill_method"].isna()]
        pure_gaps = pure_gaps[["value"]]
        pure_gaps["label"] = self.settings.gap["gaps_info"]["gap"]["outlier_flag"]
        pure_gaps["toolkit_representation"] = "gap"

        # 2. gaps records that are filled
        filled_gaps = gapsdf.loc[~gapsdf["value"].isna()]
        filled_gaps = filled_gaps[["value", "fill_method"]].rename(
            columns={"fill_method": "label"}
        )
        filled_gaps["toolkit_representation"] = "gap fill"

        # 3. gaps records that could not be filled
        partially_filled_gaps = gapsdf.loc[
            (gapsdf["value"].isna()) & (~gapsdf["fill_method"].isna())
        ]
        partially_filled_gaps = partially_filled_gaps[["value", "fill_method"]].rename(
            columns={"fill_method": "label"}
        )
        # TODO: How to label these records?
        partially_filled_gaps["label"] = self.settings.gap["gaps_info"]["gap"][
            "outlier_flag"
        ]
        partially_filled_gaps["toolkit_representation"] = "gap"

        combined_gapsdf = pd.concat([pure_gaps, filled_gaps, partially_filled_gaps])
        ...
        # Remove gaps from df
        df = df[~df.index.isin(combined_gapsdf.index)]

        if overwrite_outliers_by_gaps_and_missing:
            outliersdf = outliersdf.drop(index=combined_gapsdf.index, errors="ignore")
    ...
    def coarsen_time_resolution(
        ...
        if (
            self._applied_qc[
                self._applied_qc["checkname"] != "duplicated_timestamp"
            ].shape[0]
            > 0
        ):
            logger.warning(
