...
class Dataset:
    ...
    def combine_all_to_obsspace(self):
        ...
        df.update(other=outliersdf, join="left", overwrite=True, errors="ignore")
    ...
    def get_qc_stats(self, obstype="temp", stationnames=None, make_plot=True):
        ...
        record_check = {
            key: item["label_columnname"]
            for key, item in self.settings.qc["qc_checks_info"].items()
            if item["apply_on"] == "record"
        }
    ...
    def update_outliersdf(self, add_to_outliersdf):
        ...
        new_performed_checks_columns = list(
            set([col for col in add_to_outliersdf.columns if col.endswith("_label")])
        ...
        self.outliersdf[new_performed_checks_columns] = self.outliersdf[
            new_performed_checks_columns
        ].fillna(value="not checked")
    ...
    for column in df.columns:
        ...
        if bool_series.values.any():
            if bool_series.values.all():
                logger.warning(
                    f"No values for {column}, they are initiated\
                               as Nan."
                )
            else:
                outliers = bool_series[bool_series]
                logger.warning(
                    f"No values for stations: \
