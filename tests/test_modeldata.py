...
def create_dataset_with_modeldata():
    ...
    era5_file = datadir.joinpath("ERA5-land_timeseries_data_of_full_dataset_28_stations.csv")
...
class TestModelDataImport:
    ...
    def test_import_era5_data_from_file(self):
        ...
        assert "value" in modeldatadf.columns, "Model data DataFrame should have 'value' column"
        assert 'details' in modeldatadf.columns, "Model data DataFrame should have 'details' column"
        
        # Check that stations have model data
        stations_with_modeldata = [sta for sta in dataset.stations if not sta.modeldatadf.empty]
        assert len(stations_with_modeldata) > 0, "At least some stations should have model data"
        ...
        assert len(sta_modeldf.index.levels) == 2, "Model data should have at least one index level"
        assert "value" in sta_modeldf.columns, "Model data DataFrame should have 'value' column"
        assert 'details' in sta_modeldf.columns, "Model data DataFrame should have 'details' column"
    ...
    def test_modeltimeseries_properties(self):
        ...
        assert hasattr(modeltimeseries, 'series'), "ModelTimeSeries should have series attribute"
        assert modeltimeseries.modelname == 'ERA5-land', "ModelTimeSeries should have modelname 'ERA5-land'"
        assert modeltimeseries.modelvariable == 'temperature_2m', f"ModelTimeSeries should have modelvariable 'temperature_2m'"

        assert modeltimeseries._id() == 'vlinder04ERA5-land_temperature_2m'


        assert 'value' in modeltimeseries.df.columns, "ModelTimeSeries df should have 'value' column"
        assert 'model' in modeltimeseries.df.columns, "ModelTimeSeries df should have 'model' column"

        assert modeltimeseries.stationname == 'vlinder04', "ModelTimeSeries stationname should match station name"
...
class TestModelDataManagers:
    ...
    def test_gee_dataset_managers_availability(self):
        ...
        assert hasattr(metobs_toolkit, 'default_GEE_datasets'), "Should have default_GEE_datasets"
        
        default_datasets = metobs_toolkit.default_GEE_datasets
        assert isinstance(default_datasets, dict), "default_GEE_datasets should be a dict"
        ...
        assert hasattr(era5_manager, 'modelobstypes'), "ERA5 manager should have modelobstypes"
        assert isinstance(era5_manager.modelobstypes, dict), "modelobstypes should be a dict"
        
        # Test that manager has temperature obstype
        assert 'temp' in era5_manager.modelobstypes, "ERA5 manager should have temp obstype"
