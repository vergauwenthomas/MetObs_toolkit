...
from metobs_toolkit.backend_collection.errorclasses import MetObsModelDataError, MetObsDataAlreadyPresent
...
class TestStationModelDataMethods:
    ...
    # to pass to the solutionfixer
    solkwargs = {"testfile": Path(__file__).name, "classname": "teststationmodeldatamethods"}
    solutionfixer = SolutionFixer(solutiondir=solutionsdir)
    ...
    def test_add_to_modeldata_basic(self):
        ...
        station, model_ts1, model_ts2, model_ts3 = self.create_test_station_with_multiple_modeldata()
        ...
        assert len(station.modeldata) == 1, "Station should have 1 model data after first addition"
        
        # Add second model data (different modelname)
        station.add_to_modeldata(model_ts2, force_update=False)
        assert len(station.modeldata) == 2, "Station should have 2 model data after second addition"
        
        # Add third model data (different modelvariable)
        station.add_to_modeldata(model_ts3, force_update=False)
        assert len(station.modeldata) == 3, "Station should have 3 model data after third addition"
    ...
    def test_add_to_modeldata_force_update(self):
        ...
        assert len(station.modeldata) == 1, "Should still have only 1 model data after force update"
    ...
    def test_add_to_modeldata_wrong_type(self):
        ...
        with pytest.raises(TypeError, match="new_modeltimeseries must be an instance of ModelTimeSeries"):
    ...
    def test_get_modeltimeseries_by_obstype_only(self):
        ...
        assert retrieved._id() == model_ts1._id(), "Should retrieve the correct model data"
        assert retrieved.modelname == 'ERA5', "Should have correct modelname"
        assert retrieved.modelvariable == 'temperature_2m', "Should have correct modelvariable"
    ...
    def test_get_modeltimeseries_by_modelvariable(self):
        ...
        retrieved = station.get_modeltimeseries('temp', modelvariable='skin_temperature')
        assert retrieved._id() == model_ts3._id(), "Should retrieve skin_temperature model data"
        assert retrieved.modelvariable == 'skin_temperature', "Should have correct modelvariable"
    ...
    def test_get_modeltimeseries_by_both_filters(self):
        ...
        retrieved = station.get_modeltimeseries('temp', modelname='ERA5', modelvariable='temperature_2m')
        assert retrieved._id() == model_ts1._id(), "Should retrieve ERA5 temperature_2m model data"
        assert retrieved.modelname == 'ERA5', "Should have correct modelname"
        assert retrieved.modelvariable == 'temperature_2m', "Should have correct modelvariable"
        
        # Test another combination
        retrieved2 = station.get_modeltimeseries('temp', modelname='ERA5', modelvariable='skin_temperature')
        assert retrieved2._id() == model_ts3._id(), "Should retrieve ERA5 skin_temperature model data"
    ...
    def test_get_modeltimeseries_no_match(self):
        ...
        with pytest.raises(MetObsModelDataError, match="No model data found"):
            station.get_modeltimeseries('temp', modelname='ERA5', modelvariable='nonexistent_var')
    ...
    def test_modeldata_property_list_format(self):
        ...
        station, model_ts1, model_ts2, _ = self.create_test_station_with_multiple_modeldata()
        ...
        for item in modeldata_list:
            assert isinstance(item, metobs_toolkit.ModelTimeSeries), "Items should be ModelTimeSeries"
