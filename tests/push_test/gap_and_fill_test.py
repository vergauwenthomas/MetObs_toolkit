...
solution = {
    "temp": {
        (
            "vlinder03",
            pd.Timestamp("2022-10-07 11:00:00+0200", tz="Europe/Brussels"),
        ): 15.333333333333334,
        (
            "vlinder03",
            pd.Timestamp("2022-10-07 12:00:00+0200", tz="Europe/Brussels"),
        ): 16.566666666666666,
    },
    "temp_final_label": {
        (
...
assert (
    dataset.missing_fill_df.eq(pd.DataFrame(solution)).all().all()
...
assert (
    comb_gaps["temp"].eq(dataset.gapfilldf["temp"]).all()
), "Something wrong with the filled gaps in the combined df"
assert (
    comb_missing["temp"].eq(dataset.missing_fill_df["temp"]).all()
