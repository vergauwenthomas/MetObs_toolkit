...
class Gap:    
    ...
    def get_leading_trailing_debias_periods(self, station, obstype):
        # get debias periods
        
        leading_period, trailing_period = create_leading_trailing_debias_periods(
                                            station=station,
                                            gap=self,
                                            debias_period_settings=Settings.gaps_fill_settings['model_debias']['debias_period'],
                                            obstype=obstype)
...
class Gap_collection:
    ...
    def apply_debias_era5_gapfill(self, dataset, eraModelData, obstype='temp'):
        expanded_gabsidx_obsspace = pd.MultiIndex(levels=[['name'],['datetime']],
                                 codes=[[],[]],
                                 names=[u'name', u'datetime'])
        filled_gaps_series = pd.Series(data=[], index=expanded_gabsidx_obsspace,
        ...
        for gap in self.list:
            ...
            if (leading_obs.empty | trailing_obs.empty):
                ...
                gap.gapfill_technique = 'debias era5 gapfill (not possible)'
