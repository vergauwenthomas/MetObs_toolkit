...
class SensorWhiteSet:
    ...
    def __init__(self, white_timestamps: List = [], all_timestamps: bool = False) -> None:
    ...
    def catch_white_records(self, outliers_idx: pd.DatetimeIndex) -> pd.DatetimeIndex:
        ...
        if self.has_whites():
            ...
            if self.all_timestamps_white():
                ...
                logger.debug("All timestamps whitelisted, removing all %s outliers", len(outliers_idx))
                ...
                logger.debug("Filtered out %s whitelisted outliers, %s outliers remain", n_filtered, len(outliers))
        else:
            # no whites
            outliers = outliers_idx
            logger.debug("No whitelist applied, all %s outliers retained", len(outliers))
...
class WhiteSet:
    ...
    def test_white_records(self) -> None:
        ...
        logger.debug("Validating WhiteSet structure with levels: %s", self.white_records.names)
    ...
    def get_info(self, printout: bool = True) -> Union[str, None]:
        ...
        if self.is_empty():
            ...
            infostr += printing.print_fmt_line("Empty WhiteSet (no whitelisted records)")
        else:
            # Basic information
            infostr += printing.print_fmt_section("Whitelist details")
            n_records = len(self.white_records)
            ...
            if 'name' in levels:
                ...
                infostr += printing.print_fmt_line(f"Stations ({n_stations}): {', '.join(map(str, stations))}")
            
            if 'obstype' in levels:
                n_obstypes = self.white_records.get_level_values('obstype').nunique()
                obstypes = sorted(self.white_records.get_level_values('obstype').unique())
                infostr += printing.print_fmt_line(f"Observation types ({n_obstypes}): {', '.join(map(str, obstypes))}")
            ...
            if 'datetime' in levels:
                ...
                infostr += printing.print_fmt_line(f"Time range: {min_time} to {max_time}", 1)
            else:
                infostr += printing.print_fmt_line("All timestamps whitelisted (no 'datetime' level)")
    ...
    def create_sensorwhitelist(self, trg_station: str, trg_obstype: str) -> SensorWhiteSet:
        ...
        if 'name' in trg_whitelist.names:
            if trg_station  in trg_whitelist.get_level_values('name'):
                ...
                logger.debug("Filtered whitelist by station name, %s records remain", len(trg_whitelist))
            else: 
                # name is specified, but no matches in whitelist
                logger.debug("Station '%s' not found in whitelist, returning empty SensorWhiteSet", trg_station)
                return SensorWhiteSet(white_timestamps=[],
                                       all_timestamps=False)
        ...
        if 'obstype' in trg_whitelist.names:
            if trg_obstype in trg_whitelist.get_level_values('obstype'):
                ...
                logger.debug("Filtered whitelist by obstype, %s records remain", len(trg_whitelist))
            else:
                # obstype is specified, but no matches in whitelist
                logger.debug("Obstype '%s' not found in whitelist, returning empty SensorWhiteSet", trg_obstype)
                return SensorWhiteSet(white_timestamps=[],
                                       all_timestamps=False)
        ...
        if 'datetime' in trg_whitelist.names:
            ...
            logger.debug("Created SensorWhiteSet with %s unique timestamps", len(white_datetimes))
            ...
            logger.debug("No datetime level in whitelist, all timestamps whitelisted for this sensor")
            return SensorWhiteSet(white_timestamps=[], 
                                   all_timestamps=True)