    ...
    for wrapsta in wrappedsensors:
        if wrapsta.name not in outliers.get_level_values("name").unique():
            ...
            sta_outlier_dts = pd.DatetimeIndex(
                outliers[
                    outliers.get_level_values("name") == wrapsta.name
            ...
            remaining_dts = sensorwhiteset.catch_white_records(
                outliers_idx=sta_outlier_dts
            )
            ...
            wrapsta.update_whitelist_details(
                whitelistseries=detail_series, iteration=iteration, is_saved=True
            )

            # Create detail messages for records not saved by whitelist
            detail_series = pd.Series(
                [
                    f"Not saved by whitelist at iteration {iteration}"
                    for _ in remaining_dts
                ],
                index=remaining_dts,
            )
            wrapsta.update_whitelist_details(
                whitelistseries=detail_series, iteration=iteration, is_saved=False
            )
